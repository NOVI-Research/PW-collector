...
for api in apis:
    if api['location'] is not None:
        if api['location']['location_country_code'] is not None:
...
def location_string(location):
    location_bits = list()
    if 'location_city' in location.keys() and location['location_city'] is not None:
        location_bits.append(location['location_city'])
    else:
        return None

    if 'location_country_code' in location and location['location_country_code'] == 'USA':
        if 'location_region' in location.keys() and location['location_region'] is not None:
            location_bits.append(location['location_region'])
        else:
            return None

    if 'location_country_code' in location.keys() and location['location_country_code'] is not None:
